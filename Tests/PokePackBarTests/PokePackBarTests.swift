...
/// 포크한 앱이 원본 개발자의 연락처와 저장소를 가리키면, 테스터의 버그 리포트가
/// 모르는 사람에게 가고 업데이트가 남의 앱을 받는다. 값이 비어 있는 것은 괜찮지만
/// 원본을 가리키는 것은 안 된다.
final class AppIdentityTests: XCTestCase {
    func testDoesNotPointAtUpstreamProject() {
        XCTAssertNotEqual(SupportMail.address, "parkdongmin123@gmail.com")
        if let repo = AppLinks.githubRepo {
            XCTAssertFalse(repo.lowercased().hasPrefix("chattymin/"), "업스트림 저장소를 가리킨다")
        }
    }

    /// 수신 주소가 없으면 mailto 를 만들지 않는다 — 설정 화면이 그 항목을 감춘다.
    func testNoMailtoWithoutAddress() {
        XCTAssertNil(SupportMail.mailtoURL(to: nil, subject: "s", body: "b"))
    }

    /// 저장소가 정해지기 전에는 업데이트 확인이 쉰다.
    func testUpdatesStayOffUntilRepoIsSet() {
        XCTAssertEqual(AppLinks.updatesConfigured, AppLinks.githubRepo != nil)
    }
}

final class SupportMailTests: XCTestCase {
    /// 검증 대상은 인코딩이다. 수신 주소는 배포마다 다르므로 명시해서 분리한다.
    private static let testAddress = "tester@example.com"

    func testMailtoURLEncodesSubjectAndBody() throws {
        let url = try XCTUnwrap(SupportMail.mailtoURL(
            to: Self.testAddress,
            subject: "[PokePackBar] 문제 리포트 (v2.3.3)",
            body: "문제 내용:\n(설명)\n---\n앱 버전: v2.3.3"))
        let s = url.absoluteString
        XCTAssertTrue(s.hasPrefix("mailto:\(Self.testAddress)?"), s)
    ...
    func testMailtoEncodesPlusSign() throws {
        let url = try XCTUnwrap(SupportMail.mailtoURL(to: Self.testAddress,
                                                      subject: "C++ crash", body: "path a+b\n2+2"))
        let s = url.absoluteString
    ...
    func testMailBodyContainsDiagnostics() {
        ...
        XCTAssertTrue(body.contains("PokePackBar.log"))
    }
}
