...
final class UsageStore {
    ...
    var highestLimitUtilization: Double? {
        ...
        if usedToday.contains("codex") {
            for bucket in codexLimits?.visibleSnapshots ?? [] {
                ...
                // individualLimit is a $ spend cap — intentionally omitted (bonusEligibleWindows parity).
            }
        }
    ...
    /// 보너스 팩 지급 대상 한도 창 — 세션급(≈5h)=1개, 주간급=5개, 전 프로바이더. 공식 한도 신호가 없는
    ...
    func refresh(scheduleEmptyRetry: Bool = true) async {
        ...
        let activity = ProcessInfo.processInfo.beginActivity(
            options: .userInitiatedAllowingIdleSystemSleep, reason: "PokePackBar usage refresh")
        defer {
