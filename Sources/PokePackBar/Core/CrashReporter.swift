...
///
/// crash.log 의 기록은 **다음 실행 때 메인 로그로 합쳐 비운다**(사용자는 PokePackBar.log 한 곳만 봐도 됨).
enum CrashReporter {
    private static let logsDir: URL =
        FileManager.default.urls(for: .libraryDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("Logs")
    private static var markerURL: URL { logsDir.appendingPathComponent("PokePackBar.running") }
    /// 크래시-시점 기록 전용(회전 안 함). 시그널/예외 핸들러가 async-signal-safe 하게 append.
    private static var crashLogURL: URL { logsDir.appendingPathComponent("PokePackBar.crash.log") }
    ...
    static func install(version: String) {
        ...
        if FileManager.default.fileExists(atPath: markerURL.path) {
            AppLog.write("⚠️ 직전 세션이 정상 종료되지 않았습니다(크래시·OOM·강제종료 추정). "
                + "원인 스택은 ~/Library/Logs/DiagnosticReports/PokePackBar-*.ips 참조.")
        }
        try? Data().write(to: markerURL, options: .atomic)   // 이번 세션 running 마커
        AppLog.write("launch: PokePackBar \(version) 시작")
