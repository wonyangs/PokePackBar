...
## 표시·UI
- **숨겨야 하는 값은 「숨기는 쪽」이 아니라 「묻는 쪽」을 하나로 막는다.** 오리파는 봉투 번호와 카드의
  대응을 미리 굳혀 두는데, 공지 보드가 박스에 든 카드를 전부 늘어놓으면서 카드 칸 툴팁에 봉투 번호를
  적었다 — **덮인 봉투까지 전부.** 목록에서 1등을 찾아 번호를 읽고 그 봉투를 누르면 끝이라 고르는
  일이 통째로 사라졌다. 뷰에서 `cards.enumerated()` 로 번호를 손에 넣은 것이 원인이고, 그러면 어느
  분기에서 그 값을 쓰는지는 사람이 매번 기억해야 한다. 이제 번호를 묻는 통로가 `Oripa.revealedEnvelope`
  하나이고 **안 연 봉투에는 nil 을 준다** — 뷰가 손에 넣을 수 없는 값은 샐 수 없다. 같은 부류로
  `OripaBox.card(at:)` 도 안 연 봉투에 nil 을 준다. 회귀 가드는 `testUnopenedEnvelopeNumbersNeverLeak`
  이고, 통로를 `firstIndex(of:)` 로 되돌려 실제로 실패하는 것까지 확인했다.
  - 일반화: **비공개 상태를 뷰에 통째로 넘기고 뷰가 알아서 가리게 하지 않는다.** 공개해도 되는 것만
    돌려주는 접근자를 두고, 뷰에는 원본 배열의 첨자 접근을 주지 않는다.
...
- **「값이 내려갔다」를 「초기화됐다」로 읽으면 계정을 오가는 것만으로 보상이 복제된다.** 보너스 팩은
  한도 창 key(`claude.fiveHour`)마다 지급 표시를 두고 **사용률이 100% 아래로 내려가면 표시를 지워
  다시 무장**했다. 그 규칙은 창이 시간이 지나 초기화되는 경우만 상정한 것이다. 계정을 바꾸면 같은 key 의
  사용률이 0% 로 떨어지므로 무장이 풀리고, 원래 계정으로 돌아오는 것만으로 **같은 한도 창이 다시
  지급**된다 — 두 계정을 오가면 팩이 무한히 늘어난다. 두 계정이 모두 100% 면 전환할 때마다 지급된다.
  규율은 셋이다:
  ① 외부에서 오는 수치는 **판(instance)을 만들어 세라** — 「누구의 것인가 + 언제 초기화되는가」.
  값의 오르내림은 판을 대신하지 못한다. 여기서는 계정 이메일과 `resets_at` 을 합쳐 판으로 삼는다
  (`UsageStore.instance`).
  ② 지급 기록은 창마다 **하나가 아니라 목록**이다. 마지막 판만 적어 두면 두 계정이 번갈아 덮어써
  전환마다 다시 지급된다. 판은 초기화 시각이 지나면 다시 나타나지 않으므로 오래된 것부터 버려도 안전하다
  (`WalletStore.grantMemory`).
  ③ 판을 만들 근거가 없으면(초기화 시각을 못 얻는 프로바이더) **판이 없다고 하고** 예전 규칙에 맡긴다.
  소유자만으로 판을 만들면 값이 영원히 같아져 실제 초기화에도 다시 지급되지 않는다.
  기록 형식을 바꿀 때는 **옛 세이브를 물려받는 경로**를 같이 둔다 — 「지급했다」는 사실만 있고 어느
  판이었는지가 없으므로, 지금 관측된 판을 그 자리에 적어 두지 않으면 업데이트 직후 한 번 더 지급된다
  (`WalletStore.adoptLegacyGrantMarks`).
  회귀 가드는 `testSwitchingAccountsDoesNotPayTheSameWindowTwice` 와
  `testTwoFullAccountsPayOncePerWindowNotPerSwitch`. 예전 규칙을 되살려 실제로 실패하는지 확인했다
  (10개여야 할 팩이 20개). 반대 방향도 같이 잠근다 — `testANewWindowInstancePaysAgain` ·
  `testResetWhileStillFullStillPays` 가 없으면 「두 번 주지 않는다」를 「한 번만 주고 끝」으로
  고쳐도 통과한다.
  같은 부류를 훑을 때 **재화 장부는 무사했다** — 그쪽은 서버 한도가 아니라 디스크의 로컬 사용 로그를
  읽으므로 계정을 바꿔도 값이 내려가지 않는다. 판정 기준이 외부 계정 상태인 곳만 이 결함을 갖는다.

## 보상 크기 (무작위 대상 × 고정 수량)

- **무작위로 고른 대상에 수량을 고정하면, 대상 간 값 차이가 그대로 보상 배율이 된다.** 한도 달성
  보너스는 「무작위 세트 10팩」이었다. 팩값은 세트마다 271만~19.8억으로 **700배** 갈리므로 한 번에
  12.2억원이 나갔다 — 하루 사료 수입의 668%. 개수는 눈에 보이는 숫자라 「10개면 적당하다」로 읽히지만,
  값은 대상이 정한다. 규율은 둘이다:
  ① 보상은 **값으로 정하고 수량은 거기서 나눈다**(`PackConfig.bonusBudget`). 무작위는 「어느
  세트냐」에만 남기고 「얼마냐」에서는 뺀다.
  ② 예산으로 하나도 못 사는 대상은 **후보에서 뺀다**. 남겨 두고 「최소 1개」로 처리하면 그 대상이
  걸리는 순간 예산의 수십 배가 나가고, 고치려던 문제가 그대로 돌아온다. 하나도 못 사면 제일 싼
  것으로 1개를 준다.
  크기를 정할 땐 **그 게임의 수입에 대 보라** — 절대 금액은 감이 안 온다. 여기서는 하루 수입의
  10% 안팎(1회 1,736만원, 하루 9%)으로 잡았고, 고치기 전은 668% 였다.
  회귀 가드는 규칙(`testBonusIsWorthTheBudgetWhicheverSetIsPicked` ·
  `testSetsAboveTheBudgetAreNeverPicked`)과 **번들 데이터**(`testBonusPacksStayInBudgetForRealSets`)
  양쪽에 둔다. 시세는 갱신될 때마다 움직이므로 규칙만 잠그면 어느 날 후보가 비어도 아무도 모른다.

## 통로를 지우면 보상이 한 통로로 몰린다

- **자동 생성 경로를 걷어낼 때, 그 경로만 쓰던 계산도 같이 죽는다.** 도감 보상에는 통로가 넷
  있었다(팩·토큰·쿠폰·확정 카드). 그중 토큰·쿠폰·카드를 채우는 것은 `fill_reward` 하나였고,
  그 함수를 부르는 곳은 **자동 생성 도감 경로뿐**이었다. 테마 도감 전부를 손제작으로 바꾸면서
  그 경로를 지웠더니, 155개가 전부 `assign_reward_packs`(팩만 채운다)를 타 **131개가 똑같이
  팩만 주는 보상**이 됐다. 컴파일도 테스트도 통과한다 — 통로가 사라진 것은 데이터의 모양이라
  「보상이 비어 있지 않다」만 보는 가드에 걸리지 않는다. 경로를 걷어낼 때는 그 경로에서만
  불리던 함수를 훑고, **남은 경로가 모든 통로를 덮는지** 확인한다. 가드는 「보상이 있다」가
  아니라 **「통로가 몇 갈래로 갈리는가」**를 세야 한다.

- **고정 예산을 손으로 적어 둔 항목은 항목 수가 늘면 근거를 잃는다.** 누적 혜택은 상한이
  고정이다(한 번 얻으면 이후 모든 개봉에 걸리므로). 도감이 25개일 때 손으로 24개에 적어 둔
  값이 155개가 된 뒤에도 그대로 남아, **어느 24개가 혜택을 갖는지에 아무 근거가 없어졌고**
  나머지 131개는 영구 혜택을 받을 길이 사라졌다. dustBonus 는 그 24개가 상한을 100% 먹어
  계단 여섯 칸에 0% 가 갔다. 규율은 둘이다:
  ① 값은 원본에 적지 않고 **완성 비용에서 계산한다**(`assign_perks`). 155개를 비용 순으로
  손으로 맞출 수는 없다. 원본에 옛 값이 남아 있으면 조용히 무시하지 말고 빌드를 세운다.
  ② 계단 같은 **후순위 수령자의 몫은 먼저 떼 둔다**(`LADDER_SHARE`). 「남는 것을 준다」로
  두면 앞의 항목이 늘어나는 순간 뒤가 0 이 되고, 그건 상한을 넘지 않으므로 검사에 안 걸린다.

- **담을 수 없는 값을 통로에 적으면 화면이 거짓이 된다.** 제일 비싼 도감은 목표 회수액이
  427억원인데 그것을 담을 통로가 없다 — 카드는 세상에서 제일 비싼 것이 $20,000, 쿠폰은 20장이
  상한이다. 남은 것을 토큰으로 채우자 도감 하나가 100억원을 주게 됐고(사료를 벌 이유가
