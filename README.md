...
- **상점** — 세트별 카드팩을 산다(1999년 초판부터 최신 세트까지 10종).
  **오리파**는 여러 세트의 상위 등급만 골라 담은 100장짜리 박스다. 안에 무엇이 남았는지
  전부 보이고, 한 장 뽑으면 그 카드는 박스에서 빠진다
- **팩** — 산 팩을 뜯는다. 한 장씩 크게 나오고, 등급에 따라 카드 뒤에서 빛이 퍼진다.
  10장 모든 칸이 추첨이라 어느 자리에서도 상위 등급이 나올 수 있고, 그중 한 장은 레어 이상 확정이다.
  300팩에 한 번꼴로 전 칸이 레어 이상인 **갓팩**이 나온다
...
- **도감** — 카드 몇 장을 묶은 조합을 완성한다. 「불꽃도마뱀의 성장기」, 「로켓단 독가스반」처럼
  25종이 있고, 다 모으면 팩과 영구 혜택(적립 증가·팩 할인 등)을 받는다

공식 사용 한도(5시간·주간)를 다 채우면 보너스 팩을 10개 받는다.
